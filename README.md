# V6.0.0 本地课程学习 Agent（已归档）

> **项目已于 2026-09-19 停止开发，仓库仅作为代码与设计归档保留。**
>
> 这是一个由一名大二学生在课业之外尝试搭建的个人本地学习 Agent：我希望把课堂录音、PPT、
> 笔记、作业、错题与复习串成一条能回到原始证据的学习链路。做到这里时，我发现腾讯 ima 的
> ima Copilot 已经覆盖了其中大量更适合由成熟产品长期维护的能力。以个人的时间、精力和资源，
> 继续独立追赶同类基础能力已经不再是合理的选择，因此决定在这里结束项目。
>
> 这不是对产品或团队的指责。若 [@Tencent](https://github.com/Tencent) 或 ima 团队看到这份
> 归档，希望它至少能留下一个学生认真尝试构建个人学习 Agent 的过程、取舍与问题记录。
>
> 本仓库已做脱敏处理：不包含课程材料、SQLite 正式数据库、模型/API 密钥、移动访问 Token、
> 本地网关数据库、主密钥、日志或生成产物。仓库未附带开源许可；除 GitHub 平台允许的查看与
> fork 外，复用代码前请先联系作者。

有关停止原因、保留内容和接手边界，请阅读 [PROJECT_ARCHIVED.md](PROJECT_ARCHIVED.md)。
