...
const suggestions: Array<{ path: string; label: string; icon: string; desc: string }> = [
  { path: '/', label: '总览', icon: '🎯', desc: '首页仪表盘' },
  { path: '/courses', label: '课程与考试', icon: '📚', desc: '管理课程与考试日历' },
  { path: '/runs', label: '运行日志', icon: '📜', desc: '查看最近工作流运行' },
  { path: '/graph', label: '知识图谱', icon: '🕸️', desc: '浏览知识节点关系' },
]
</script>
  ...
  <div class="page-shell">
    ...
    <section class="section text-center">
      ...
      <p class="mt-2 text-sm text-slate-400">
        当前 URL：<code class="rounded bg-slate-900 px-2 py-1 font-mono text-xs">{{ $route.fullPath }}</code>
      </p>
